...
const ProductCard: React.FC<ProductCardProps> = ({ product, className = "" }) => {
    ...
    for (let i = 1; i <= 5; i++) {
      if (i <= fullStars) {
        stars.push(
          <Star
            key={i}
            className="w-5 h-5 text-app-main fill-app-main"
          />
        );
  ...
  return (
    <Link to={`/product/${product._id}`} className="block group">
      <div className={`bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden hover:shadow-lg transform hover:-translate-y-1 transition-all duration-300 ${className}`}>
        {/* Image Section */}
